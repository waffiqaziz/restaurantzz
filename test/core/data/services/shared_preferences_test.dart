...
void main() {
  ...
  group('SharedPreferencesService', () {
    test('saveSettingValue_shouldSaveSettings', () async {
      ...
      verify(() => mockSharedPreferences.setBool(SharedPreferencesService.keyNotification, true))
          .called(1);
      verify(() => mockSharedPreferences.setBool(SharedPreferencesService.keyIsDark, false))
          .called(1);
    });
    ...
    test('setTheme_shouldThrowException_whenSetBoolFails', () async {
      ...
      expect(() => sharedPreferencesService.setTheme(false), throwsException);
    ...
    test('getSettingValue_shouldReturnCorrectSettings', () {
      when(() => mockSharedPreferences.getBool(SharedPreferencesService.keyNotification))
          .thenReturn(true);
      when(() => mockSharedPreferences.getBool(SharedPreferencesService.keyIsDark))
          .thenReturn(false);
    ...
    test('isDarkModeSet_shouldReturnTrueIfKeyExists', () {
      when(() => mockSharedPreferences.containsKey(SharedPreferencesService.keyIsDark))
          .thenReturn(true);
    ...
    test('saveSettingValue_shouldThrowExceptionWhenFails', () async {
      ...
      when(() => mockSharedPreferences.setBool(any(), any()))
          .thenThrow(Exception("Shared preferences cannot save the setting value."));

      expect(() => sharedPreferencesService.saveSettingValue(setting), throwsA(isA<Exception>()));
    });
  });
}
