...
void main() {
  group('LocalNotificationProvider', () {
    ...
    test('showNotification_shouldTriggerNotification', () {
      when(
        () => mockService.showNotification(
          ...
          body: "This is a new notification with payload rqdv5juczeskfw1e867:list",
    ...
    test('scheduleDailyElevenAMNotification_shouldScheduleNotification', () {
      when(
        () => mockService.scheduleDailyElevenAMNotification(
          id: any<int>(named: 'id'),
    ...
    test('scheduleTestNotification_shouldScheduleNotification', () {
      ...
      verify(() => mockService.scheduleTestNotification(id: any<int>(named: 'id'))).called(1);
    });
    ...
    test('checkPendingNotificationRequests_shouldUpdatePendingList', () async {
      when(() => mockService.pendingNotificationRequests()).thenAnswer(
        (_) async => [PendingNotificationRequest(1, 'Test Notification', 'Test Body', 'Payload')],
      );
      ...
      expect(isNotified, isTrue, reason: "Expected notifyListeners to be called.");
      expect(localNotificationProvider.pendingNotificationRequests.length, 1);
      expect(localNotificationProvider.pendingNotificationRequests.first.id, equals(1));
    ...
    test('cancelNotification_validId_shouldCancelNotification', () async {
      ...
      when(() => mockService.cancelNotification(notificationId)).thenAnswer((_) async => {});
